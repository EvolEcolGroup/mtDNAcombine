...
```{r echo=FALSE}
knitr::include_graphics(system.file("extdata", "/mtDNAcombine_flow.png", 
...
```{r install_mtDNAcombine, eval=FALSE}
...
```{r manual install older deps, eval=FALSE}
...
```{r summary of species available}
GB_species <- droplevels(as.data.frame(unique(GB_data$sci_nam)))
```

At the moment:
```{r echo=FALSE}
print(paste("GB_genes has", nrow(GB_genes), " unique gene names while GB_species has", 
            nrow(GB_species), "unique species names"))
```
...
```{r echo=FALSE}
colnames(GB_species) <- "Unique Names"
print(GB_species, row.names=FALSE)
```
...
```{r find anything that looks like a synonym}
...
```{r remove suprious subspecies names}
...
```{r summarise species available after clean up, echo=FALSE}
GB_species <- droplevels(as.data.frame(unique(GB_data$sci_nam)))
...
```{r filter df to gene of interest}
...
```{r get sequence data into dataframe}
...
```{r export summary data and sequence data for each species}
...
```{r align and summarise the sequence data}
...
```{r echo=FALSE}
knitr::include_graphics(system.file("extdata", "/hist_Pinicola_enucleator.png",
                                     package="mtDNAcombine"))
```
...
```{r echo=FALSE}
knitr::include_graphics(system.file("extdata", "/hist_Calidris_maritima.png",
                                     package="mtDNAcombine"))
```
...
```{r echo=FALSE}
knitr::include_graphics(system.file("extdata","/Net_Picoides_tridactylus.png",
                                    package="mtDNAcombine"))
```
...
```
```{r read in preexisitng info_df file, echo=FALSE}
info_df <- read.csv(system.file("extdata","Inf_df.csv", package="mtDNAcombine"), stringsAsFactors = T)
```
...
```{r echo=FALSE}
what_gets_dropped <- cbind('EU166960.1','new_Motacilla_alba')
colnames(what_gets_dropped) <- c('outlier_accession', 'spp_name')
```
```{r eval=FALSE}
...
```{r setup_basic_xml function, eval=FALSE}
...
```{r plotting median, fig.align='center', fig.dim=c(5,5)}
