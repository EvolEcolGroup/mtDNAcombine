#' Drops populations from main data frame when sample size is less than threshold value
#'
#' \code{drop_low_sample_size}
#'
...
#' @param min_sample minimum number of haplotypes before user considers data will
#' be uninformative
#'
#' @export


drop_low_sample_size <- function(info_df, min_sample) {
    if (inherits(info_df, "data.frame")) {
        ...
        if (all(colnames(info_df) %in% expected_colnames)) {
            info_df <-
                subset(info_df, !as.numeric(info_df$sample_size) < min_sample)
